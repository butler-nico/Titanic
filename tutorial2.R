...
aggregate(Survived ~ Child + Sex, data = train, FUN = sum) #sum subset
aggregate(Survived ~ Child + Sex, data = train, FUN = length) #total for each subset

#create a function that takes the subset vector as input and applies both the sum and length commands to it, and then does the division to give us a proportion
